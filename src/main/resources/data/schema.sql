-- Creates a dummy table test to resolve an error when starting the application.
...
CREATE TABLE IF NOT EXISTS confirmations (
    ...
    CONSTRAINT fk_confirmations_update_by FOREIGN KEY (updated_by) REFERENCES users (id) MATCH SIMPLE ON UPDATE CASCADE ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS credentials (
    ...
    referenced_id CHARACTER NOT NULL,
    user_id BIGINT NOT NULL,
    created_by BIGINT NOT NULL,
    updated_by BIGINT NOT NULL,
    created_at TIMESTAMP(6) WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
...
CREATE INDEX IF NOT EXISTS index_user_roles_id ON user_roles (user_id);
